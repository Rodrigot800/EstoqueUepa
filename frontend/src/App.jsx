...
export default function App() {
  ...
  return <div className="shell">
    <aside className="sidebar"><a className="brand" href="#" onClick={e => { e.preventDefault(); changePage('stock'); }}><div className="brand-icon"><Boxes size={26}/></div><span>Estoque<span className="brand-sub">UEPA</span></span></a>
      <div className="warehouse-picker"><label htmlFor="warehouse-select">ESTOQUE ATUAL</label><select id="warehouse-select" value={warehouseId} onChange={e => selectWarehouse(e.target.value)} disabled={!warehouses.length}>{!warehouses.length && <option value="">Carregando…</option>}{warehouses.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}</select><button className="secondary" onClick={() => setModal('warehouse')}><Plus size={16}/> Adicionar estoque</button></div><p className="nav-label">GESTÃO DE MATERIAIS</p>
      ...
      <div className="sidebar-bottom"><span className="university-mark">U</span><div>Universidade do Estado do Pará<small>Controle de almoxarifado</small></div></div>
    </aside>
